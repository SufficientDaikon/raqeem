...
- **كل تفريغ بيرجع بشكلين.** النص الحرفي زي ما الموديل قاله، ونسخة معالجة للعربي: توحيد الألف
  ...
  وكمان شيل الحروف المخفية اللي بتيجي مع أي نص عربي متنسّخ — zero-width joiners وعلامات الاتجاه
  و BOM تايه. `١٢٫٥` بتبقى `12.5` رقم **واحد** مش اتنين. ده الفرق بين نص إنت تقدر تقراه ونص
  برنامج يقدر يفهمه.
...
| `--api-key` | — | بيرجع لـ `$RAQEEM_API_KEY` لو مااتحطش. الأحسن تستخدم الـ env — شوف الملاحظة تحت. |
| `--model` | `cohere-transcribe-arabic-07-2026` | الافتراضي ده لـ `--provider cohere`، واللي بتشترط معرّف **مؤرّخ** (الأسماء غير المؤرّخة بترجّع 404). **مطلوب** مع `--provider openai`. |
| `--lang` | `ar` | ISO-639-1. |
| `--timeout` | `300` | بالثواني، وبتغطي الرفع + الـ inference + التنزيل. زوّدها لو الـ CPU بطيء. بتقبل من 1 لـ 86400. |
...
وبعت المفتاح من الـ environment أحسن من `--api-key` لما تقدر. أي argument بيبقى ظاهر لكل
process تانية على الجهاز (`ps` و `/proc/<pid>/cmdline`) وبينزل كمان في الـ shell history،
والـ environment variable مابتعملش ولا واحدة من الاتنين.
...
أي فشل في التفريغ بيرمي `raqeem.TranscriptionError`. و provider غلط، أو مفتاح/endpoint/model
ناقص، بيرمي `ValueError`.
...
- **تطابق المعالجة، بالكلام الصريح.** `normalize_ar` دالة واحدة متكتوبة بلغتين، ولحد 0.3.0 كان كل
  test suite فيهم بيكتب الحالات بتاعته بإيده. ده مش حماية من الفروق، وفعلاً ماحماش: نسخة Rust
  فضلت ناقصة pass كاملة عن الأصل والاتنين فاضلين خضرا، لأن مافيش ولا حالة فيها zero-width joiner.
  دلوقتي الاتنين بيقروا من نفس ملف الـ vectors المولّد، والكلام إن التنفيذين متطابقين مبني على
  المسح مش على حالات مختارة: **258,176 حالة بحرف واحد** على الـ BMP كله وعينات من فوقه،
  و**200,000 نص عشوائي متعدد الحروف** فيه عربي وتشكيل وعلامات قرآنية ونوعي الأرقام وحروف
  التحكم المخفية وكل أنواع المسافات. صفر اختلاف، وصفر فشل في الـ idempotence. المسح ده هو اللي
  لقى الـ bug الاتنين — والتاني محدش كان أصلاً بيدوّر عليه. الاختلاف الوحيد المقبول هو تحويل
  حالة الأحرف غير العربية وغير الـ ASCII، لأن جداول Unicode في Rust وفي Python مش نفس الإصدار:
  تسع حروف، ولا واحد فيهم في العربي ولا في ASCII. بص على
...
[.claude/skills/](.claude/skills/) لو بتضيف backend جديد للتفريغ — فيه skill بيمشّيك على كل
تعديل محتاجينه، بما فيه قرار المفاتيح اللي الـ compiler هيوقّفك عنده لحد ما تحدده.

لو هتبني من الـ source، محتاج Rust 1.86 أو أحدث.
